...
static __init int print_pid(void)
{
    ...
    list_for_each(pos,&task->tasks)
    {
        ...
        switch (p -> state) {
            case TASK_RUNNING:
                run_name[run_count] = p;
                run_count++;
                break;
            case TASK_INTERRUPTIBLE:
                inter_name[inter_count] = p;
                inter_count++;
                break;
            case TASK_UNINTERRUPTIBLE:
                uninter_name[uninter_count] = p;
                uninter_count++;
                break;
            case EXIT_DEAD:
                dead_name[dead_count] = p;
                dead_count++;
                break;
            case EXIT_ZOMBIE:
                zombie_name[zombie_count] = p;
                zombie_count++;
                break;
            default:
                other_name[other_count] = p;
                other_count++;
                break;
        }
    }
